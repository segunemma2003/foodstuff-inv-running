"""Dashboard domain service logic."""
...
COST_OF_SALES_PRIMARY_RECIPIENT = os.getenv(
    "COST_OF_SALES_RECIPIENT_EMAIL", "foodstuffstorepo@gmail.com"
)
...
def get_overview(db: Session) -> schemas.DashboardOverview:
    ...
    delivery_rows = (
        db.query(
            models.Invoice.delivery_type, func.sum(models.Invoice.total_amount).label("total")
        )
        .filter(_not_cancelled_invoice_filter())
...
def get_cost_of_sales_detail(
    ...
    product_rows = (
        item_q.group_by(models.InvoiceItem.product_id, models.Product.product_name)
        ...
        "by_product": [
            {
                ...
                "qty": float(r.qty),
                "unit_cost_price": (float(r.cost or 0) / float(r.qty))
...
def queue_cost_of_sales_email_report(
    db: Session,
    ...
    if date_from or date_to:
        date_label = f" ({date_from or '—'} to {date_to or '—'})"
    ...
    <div style="font-family:Arial,sans-serif;max-width:700px">
      ...
      <table style="border-collapse:collapse;width:100%;margin-bottom:24px">
        <tr style="background:#eafaf1">
          ...
          <td style="padding:10px">&#8358;{summary['total_cost']:,.2f}</td>
        </tr>
        <tr>
          <td style="padding:10px;font-weight:bold">Total Revenue</td>
          <td style="padding:10px">&#8358;{summary['total_revenue']:,.2f}</td>
        </tr>
        <tr style="background:#eafaf1">
          <td style="padding:10px;font-weight:bold">Gross Profit</td>
          <td style="padding:10px;color:#1e8449;font-weight:bold">&#8358;{summary['gross_profit']:,.2f}</td>
        </tr>
        <tr>
          <td style="padding:10px;font-weight:bold">Gross Margin</td>
          <td style="padding:10px">{summary['gross_margin_pct']:.1f}%</td>
        </tr>
      </table>
    ...
    if additional_emails:
        recipients.extend([str(email).strip() for email in additional_emails if str(email).strip()])
    ...
    log_queue_event(
        ...
        metadata={"recipients": recipients, "customer_id": customer_id, "product_id": product_id},
    )
    return {"message": f"Report queued for {len(recipients)} recipient(s)"}


def list_queue_events_with_status(
    db: Session, limit: int = 50, event_type: Optional[str] = None
) -> list[schemas.QueueEventOut]:
...
def generate_cost_of_sales_pdf_bytes(
    db: Session,
    date_from: Optional[date] = None,
    date_to: Optional[date] = None,
    customer_id: Optional[int] = None,
    ...
    return generate_cost_of_sales_pdf(data, title_suffix=label)
