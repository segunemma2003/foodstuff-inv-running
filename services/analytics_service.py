...
def sales_analytics(
    ...
    *,
    ...
    filtered_invoice_query = _inv_filters(
        db.query(models.Invoice.id.label("invoice_id")),
        date_from,
        date_to,
        delivery_type,
        payment_term,
    ...
    if market_invoice_ids_subquery is not None:
        filtered_invoice_query = filtered_invoice_query.filter(
            models.Invoice.id.in_(market_invoice_ids_subquery)
        )
    if product_invoice_ids_subquery is not None:
        filtered_invoice_query = filtered_invoice_query.filter(
            models.Invoice.id.in_(product_invoice_ids_subquery)
        )
    ...
    top_markets_rows = (
        top_markets_query.group_by(models.ProductCategory.id, models.ProductCategory.name)
        .order_by(func.sum(models.InvoiceItem.line_total).desc())
        .limit(10)
        .all()
    )
    ...
    return schemas.SalesAnalytics(
        ...
        top_customers=[
            {
                ...
                "total": float(customer_row.total or 0),
            }
        ...
        top_products=[
            {
                ...
                "total": float(product_row.total or 0),
        ...
        sales_by_delivery_type={
            (
                _enum_or_str_value(delivery_row.delivery_type)
                if delivery_row.delivery_type is not None
                else "unknown"
            ): float(delivery_row.total_amount or 0)
            for delivery_row in delivery_split_rows
        },
        sales_by_payment_term={
            (
                str(payment_term_row.payment_term)
                if payment_term_row.payment_term is not None
                else "unknown"
            ): float(payment_term_row.total_amount or 0)
        ...
        sales_by_staff=[
            {
                ...
                "total_sales": float(staff_row.total or 0),
                "invoice_count": int(staff_row.count or 0),
            }
            for staff_row in staff_sales_rows
        ],
        daily_trend=[
            {
                "date": str(daily_row.invoice_date),
                "total": float(daily_row.total or 0),
                "count": int(daily_row.count or 0),
            }
        ...
        monthly_trend=[
            {
                ...
                "total": float(monthly_row.total or 0),
                "count": int(monthly_row.count or 0),
            }
...
def customer_behavior(
    ...
    *,
    ...
    cust_q = db.query(models.Customer).filter(models.Customer.is_active)
    if customer_id:
        ...
        top_prod_by_cust[product_row.customer_id].append(
            {
                ...
                "product_name": product_row.product_name,
                "qty": float(product_row.qty),
                "value": float(product_row.value),
            }
        )
    for customer_id_key in top_prod_by_cust:
        top_prod_by_cust[customer_id_key].sort(key=lambda x: x["value"], reverse=True)
        top_prod_by_cust[customer_id_key] = top_prod_by_cust[customer_id_key][:10]
    ...
    for customer in customers:
        ...
        this_month_val = sum(float(inv.total_amount) for inv in invoices if inv.invoice_date >= this_month_start)
        last_month_val = sum(
...
def staff_performance(
    ...
    *,
    ...
    selected_market = market_id or category_id
    market_invoice_ids = _invoice_ids_for_market(db, selected_market)
    users = db.query(models.User).filter(models.User.is_active)
    if user_id:
...
def comprehensive_stats(
    ...
    *,
    ...
    for quotation_status_row in quot_status_rows:
        quotation_stats_by_status[quotation_status_row.status.value] = {
            "cnt": quotation_status_row.cnt,
            "total_amount": float(quotation_status_row.total_amount or 0),
        }
    ...
    conv_r = (
        round(q_converted / (q_approved + q_converted) * 100, 2) if (q_approved + q_converted) else 0
    )
    ...
    quot_stats = schemas.QuotationStats(
        ...
        approval_rate=approval_r,
        rejection_rate=rejection_r,
        conversion_rate=conv_r,
        total_value=q_total_amount,
    )
    ...
    i_paid = invoice_stats_by_status.get("paid", {}).get("cnt", 0) + invoice_stats_by_status.get(
        "completed", {}
    ).get("cnt", 0)
    ...
    total_billed = sum(
        status_stats["billed"]
        for status_name, status_stats in invoice_stats_by_status.items()
        if status_name != "cancelled"
    )
    total_coll = sum(
        status_stats["collected"]
        for status_name, status_stats in invoice_stats_by_status.items()
        if status_name != "cancelled"
    )
    ...
    inv_stats = schemas.InvoiceStats(
        total=i_total,
        active=i_active,
        partially_paid=i_partial,
        paid=i_paid,
        cancelled=i_cancelled,
        paid_rate=paid_rate_v,
        cancel_rate=cancel_rate_v,
    ...
    all_users = db.query(models.User).filter(models.User.is_active).all()
    ...
    uq_all = uq_base.group_by(models.Quotation.created_by, models.Quotation.status).all()
    ...
    ui_all = ui_base.group_by(models.Invoice.created_by, models.Invoice.status).all()

    quotation_counts_by_user: dict = defaultdict(dict)
    for quotation_summary_row in uq_all:
        quotation_counts_by_user[quotation_summary_row.created_by][
            quotation_summary_row.status.value
        ] = quotation_summary_row.cnt
    ...
    for u in all_users:
        ...
        by_sales.append(
            schemas.SalesPersonStats(
                user_id=u.id,
                full_name=u.full_name,
                username=u.username,
                role=u.role.value,
                quotations_total=uq_total,
                quotations_draft=uq_draft,
                quotations_pending=uq_pending,
                quotations_approved=uq_appr,
                quotations_rejected=uq_rej,
                quotations_converted=uq_conv,
                quotation_approval_rate=uq_appr_r,
                quotation_conversion_rate=uq_conv_r,
                invoices_total=ui_total,
                invoices_paid=ui_paid,
                invoices_partially_paid=ui_partial,
                invoices_active=ui_active,
                invoices_cancelled=ui_cancel,
                total_billed=ui_billed,
                total_collected=ui_coll,
                total_outstanding=ui_out,
                collection_rate=ui_coll_r,
    ...
    for manager_status_row in manager_rows:
        if manager_status_row.id not in manager_stats_map:
            manager_stats_map[manager_status_row.id] = {
                "full_name": manager_status_row.full_name,
                "username": manager_status_row.username,
                "statuses": {},
            }
        manager_stats_map[manager_status_row.id]["statuses"][manager_status_row.status.value] = {
            "cnt": manager_status_row.cnt,
            "total_amount": float(manager_status_row.total_amount or 0),
        }
    ...
    for top_sales_row in top_sales_rows:
        top_sales_by_mgr[top_sales_row.approved_by].append(
            {
                "name": top_sales_row.full_name,
                "count": top_sales_row.cnt,
                "value": float(top_sales_row.total_amount),
            }
        )
        ...
        top_customers_revenue=[
            {
                "customer_id": customer_row.id,
                "customer_name": customer_row.customer_name,
                "orders": customer_row.orders,
                "billed": float(customer_row.billed),
                "collected": float(customer_row.collected),
                "outstanding": float(customer_row.billed) - float(customer_row.collected),
                "collection_rate": (
                    round(float(customer_row.collected) / float(customer_row.billed) * 100, 1)
                    if customer_row.billed
                    else 0
                ),
            }
